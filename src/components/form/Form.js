...
function form({database, itemsInDB, push}) {
  ...
  return (
    <form className='flex border-2 border-black flex-col mx-auto p-10 rounded-md items-center max-w-xs'  onSubmit={handleSubmit}>
        <ShoppingCart/>
        <input className='bg-gray-200 focus:bg-white border border-solid border-violet-600 focus:border-blue-400 rounded-md p-2 m-2 transition duration-300 text-center' type='text' name='item' placeholder='Enter Item' onChange={handleChange}/>
